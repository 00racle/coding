...
# ================================================== U-11 ==================================================
        ...
        U11 = ["U-11", "원격 로그인 또는 원격 쉘 등 사용 설정 유무", "", "rpm -qa | grep -E 'rsh|rlogin|rexec'"]
        u11 = []
        u11s = ""
        stdin, stdout, stderr = ssh.exec_command(U11[3])
        for line in stdout:
            u11.append(line.rstrip('\n'))

        if len(u11) == 0:
            U11[2] = "양호"
            u11s = "원격 로그인 또는 원격 쉘(rsh, rlogin, rexec)가 없습니다."
        else:
            U11[2] = "취약"
            u11s = "원격 로그인 또는 원격 쉘(rsh, rlogin, rexec) 서비스가 있습니다: "
...
# ================================================== U-12 ==================================================
        ...
        U12 = ["U-12", "불필요한 네트워크 서비스 제거", "", "rpm -qa | grep -E 'tftp|talk|ntalk|finger'"]
        u12 = []
        u12s = ""
        ...
        if len(u12) == 0:
            U12[2] = "양호"
            u12s = "불필요 네트워크 서비스(tftp, talk, ntalk, finger)가 없습니다."
        else:
            U12[2] = "취약"
...
# ================================================== E-02 ==================================================
        E02 = ["E-02", "세션 타임아웃 설정 확인", "", "cat /etc/profile | grep TMOUT"]
        e02 = []
        e02s = ""

        stdin, stdout, stderr = ssh.exec_command(E02[3])
        for line in stdout:
            e02.append(line.rstrip('\n'))
        if len(e02) == 0 or e02[0].split("=")[1] == "0":
            E02[2] = "취약"
            e02s = "세션 타임아웃 설정이 되어있지 않습니다."
        elif e02[0].split("=")[1] > "300":
            E02[2] = "취약"
            e02s = "세션 타임아웃 설정이 300초 이상으로 취약합니다. 결과값: "
        else:
            E02[2] = "양호"
            e02s = "세션 타임아웃 설정이 양호합니다. 결과값: "
        ...
        nsheet.append(E02)
        if E02[2] == "취약":
            nsheet['C25'].fill = PatternFill(start_color="FFFF0000", end_color="FFFF0000", fill_type="solid")
# ================================================== E-03 ==================================================
        E03 = ["E-03", "NTP 동기화 확인", "", "/usr/sbin/ntpq -p | grep '192.168.5.115' | awk '{print $1}'"]
        e03 = []

        stdin, stdout, stderr = ssh.exec_command(E03[3])
        for line in stdout:
            e03.append(line.rstrip('\n'))
        for line in stderr:
            e03.append(line.rstrip('\n'))
        if len(e03) != 0 and '*' in e03[0]:
            E03[2] = "양호"
            e03s = "ntp 연결이 양호 합니다. 결과값: "
        else:
            E03[2] = "취약"
            e03s = "ntp 연결이 취약 합니다. 결과값: "
        ...
        nsheet.append(E03)
        if E03[2] == "취약":
            nsheet['C26'].fill = PatternFill(start_color="FFFF0000", end_color="FFFF0000", fill_type="solid")
